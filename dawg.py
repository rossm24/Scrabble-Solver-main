...
'''if __name__ == "__main__":
    ...
    # Debugging: Check if words are loading properly
    print(f"✅ Loaded {len(big_list)} words from grade5.txt")
    print("🔍 First 10 words:", big_list[:10])

    if len(big_list) == 0:
        print("🚨 ERROR: grade5.txt is empty!")
        exit()
    ...
    # Debugging: Check if DAWG was built properly
    if not root or not getattr(root, 'children', None):
        print("🚨 ERROR: The DAWG structure is empty! Check build_dawg().")
        exit()

    # Save Pickle File
    with open("lexicon/grade5.pickle", "wb") as file_handler:
        pickle.dump(root, file_handler)

    # Debugging: Check Pickle File Size
    import os
    file_size = os.path.getsize("lexicon/grade5.pickle")
    print(f"✅ Pickle file saved! Size: {file_size} bytes")
'''
