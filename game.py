...
if __name__ == "__main__":
    ...
    for player in players:
        player.letters_in_rack = random.sample(tile_bag, 7)
        [tile_bag.remove(letter) for letter in player.letters_in_rack]
    ...
    while True:
        ...
        if game_state == "game_screen":
            screen.fill((0, 0, 0))
            #MAIN GAME LOOP HINGES ON WHEN A NEW WORD IS PLAYED:
            # UPDATE SCORE, INTERACT WITH DISPENSER FOR LETTERS, CHANGE PLAYER TURN

            #if total words on the board increases
                # some interaction with the dispenser
                #player_turn = (player_turn + 1) mod 2


            #if button pressed:
            current_player = players[player_turn]
            current_player.remove_letters_from_rack(game.get_best_move(current_player))

            number_of_tiles_to_get_from_dispenser = 7 - len(current_player.letters_in_rack)
            #dispenser.fetch_tiles(number_of_tiles...)

            word_rack, new_letters = refill_word_rack(word_rack, tile_bag)
            [tile_bag.remove(letter) for letter in new_letters]
            if game.best_word == "":
                #OUTPUT TO SCREEN: NO WORDS LEFT TO PLAY
